...
function readTurnsContainer(responseData: unknown): { root: JsonRecord; container: JsonRecord; turns: unknown[]; write: (turns: unknown[]) => unknown } | null {
  ...
  if (thread && Array.isArray(thread.turns)) {
    return {
      ...
      container: thread,
      turns: thread.turns,
      write: (turns) => ({ ...root, thread: { ...thread, turns } }),
    }
  }

  const conversationState = asRecord(root.conversationState)
  if (conversationState && Array.isArray(conversationState.turns)) {
    return {
      root,
      container: conversationState,
      turns: conversationState.turns,
      write: (turns) => ({ ...root, conversationState: { ...conversationState, turns } }),
    }
  }
...
export function slimLiveStateCommandOutputs(
  ...
  const turnsContainer = readTurnsContainer(responseData)
  if (!turnsContainer) return { data: responseData, stats: emptyStats }
  ...
  const nextTurns = turnsContainer.turns.map((turn) => {
    const turnRecord = asRecord(turn)
  ...
  return {
    data: turnsContainer.write(nextTurns),
    stats: {
      blockCount,
...
export function setLiveStateDigestHeaders(res: ServerResponse, digest: string): void {
...
export function prepareLiveStateResponse(
  ...
  return {
    data: slimmed.data,
    digest,
    statusCode: digest && requestedDigest === digest && !metadata.isInProgress ? 204 : 200,
    stats: slimmed.stats,
  }
}

export function sendPreparedLiveStateResponse(res: ServerResponse, prepared: PreparedLiveStateResponse, sendJson: (statusCode: number, payload: unknown) => void): void {
  if (prepared.statusCode === 204) {
    sendLiveStateNoContent(res, prepared.digest)
    return
  }
  setLiveStateDigestHeaders(res, prepared.digest)
  sendJson(200, prepared.data)
}
...
export async function maybeSendFastLiveStateUnchanged(
  ...
  getCurrentGeneration: (threadId: string) => number = () => generation,
): Promise<boolean> {
  ...
  if (!cached.sessionPath) return false
  ...
  if (getCurrentGeneration(threadId) !== generation) return false
