...
15. In light theme, create or edit a thread automation with `Daily` at a near-future local time.
16. Confirm the sidebar row shows a concrete `Next run` timestamp instead of `Not scheduled`.
17. Wait until the scheduled minute passes.
18. Confirm `/codex-api/thread-queue-state` receives a heartbeat message for that automation.
19. Confirm an idle thread with `status.type = active` starts the queued automation turn instead of leaving the heartbeat stuck in the queue.
20. Use `Run now` from the automation manager and confirm it also queues and drains.
21. Switch to dark theme and repeat steps 15, 16, and 20 to confirm the automation manager remains readable and usable.
22. Remove the final automation and confirm the thread menu returns to `Add automation…`.
